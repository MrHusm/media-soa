...
public class LoggerProfileIntercepter {
	...
	@Around("@annotation(com.yxsd.kanshu.profile.LoggerProfile)")
		...
		if (logger.isInfoEnabled()) {
			...
			if (loggerProfile.recordTime()) {
				...
				try {
					...
					clock.stop();
					logger.warn(loggerProfile.methodNote() + "接口用时:" + clock.getLastTaskTimeMillis() + "ms");
					if (clock.getLastTaskTimeMillis() > loggerProfile.allowMaxTime()) {
						logger.warn(loggerProfile.methodNote() + "接口调用超过设定时间,用时:" + clock.getLastTaskTimeMillis() + "ms,设定默认时间：" + loggerProfile.allowMaxTime());
					}
				}
			} else {
				logger.info(JSON.toJSONString(logMap));
